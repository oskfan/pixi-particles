...
const importStatement = (/import {([^}]*)} from '@pixi\/[a-z]*';/).exec(moduleTypes);

let ambientTypes = `${moduleTypes.replace(importStatement[0], 'declare namespace particles {')}\n}`;
...
for (let i = 0; i < importedTypes.length; ++i) {
    ...
    ambientTypes = ambientTypes.replace(new RegExp(`\\b${classImport}\\b`, 'g'), `${classImport}`);
}
